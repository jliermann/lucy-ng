...
- [x] **Phase 83: Constraint-Hardness Guard (FIX-10)** - An uncertain structural inference must never become a hard, solution-excluding LSD constraint. Third instance of the recurring meta-failure (after v3.0 Pitfall 6 erosion + the FIX-07 4J trap): uncertain interpretation → hard PROP/BOND → correct structure excluded. CASE9 trigger (isopropyl 4-(1-hydroxyethyl)benzoate, C12H16O3, RDKit-verified ZERO ring-O): the nmr-chemist turned `detect neighbours 150.80`→O 81% into a hard `PROP 2 O 1`, forcing oxygen onto the aromatic C that actually carries a benzylic CH(OH)CH₃ → the true para-alkylbenzoate left the search space. Scope (narrow, constraint-hardness only): (a) nmr-chemist — detect-neighbours O/N is advisory/statistical-prior, 145–160 ppm aromatic C is ambiguous (ring-O vs EDG/benzylic substituent), re-assert Pitfall 6; (b) lsd-engineer — no hard heteroatom PROP/BOND from statistics alone, leave open, let ranking decide; (c) devils-advocate — gate blocking/flagging a hard heteroatom-neighbour PROP/BOND lacking cited direct evidence; (d) optional tooling — `detect neighbours` self-labels advisory + regression test. Out of scope (separate blockers): benzene-ring emergence (D-04), HMBC-pool richness. (depends on Phase 82) (DONE + VERIFIED 2026-06-10: 4/4 success criteria; advisory rule + G-PROP-EVIDENCE gate + dominant_element tooling shipped; CASE9 truth not excluded; suite 1071 passed. Also removed obsolete TestBadlistPatterns (FIX-09 fallout). Blind CASE9 re-run = remaining separate gate. See 83-VERIFICATION.md.) **[UAT-04 outcome: CASE9 SOLVED 2026-06-11 on Opus 4.8 — `CC(C)OC(=O)c1ccc(C(C)O)cc1` (D202 truth), RDKit-verified, rank 1 MAE 1.17. Earlier failures were substantially MODEL-driven (the prior run silently ran on Sonnet 4.6 via the CLAUDE_CODE_SUBAGENT_MODEL=sonnet override, now fixed to inherit). See CASE9-UAT-04-PASS verdict.]**
- [x] **Phase 84: Aromatize SMILES Before Prediction (FIX-11)** - `outlsd`/`lucy lsd run` emits Kekulé SMILES (aromatic ring as `C(=C1)C=CC(=C1)`); feeding them directly to `lucy predict c13` makes the HOSE/predictor treat the ring as a non-aromatic diene → wrong aromatic shifts → wrong MAE/ranking. Exposed by the 2026-06-11 Opus-4.8 CASE9 run: the solution-analyst predicted on un-aromatized Kekulé SMILES and nearly mis-escalated ("structure NOT in set, all implausible") — a false alarm the coordinator caught (the truth WAS line 12; after RDKit canonicalization MAE 1.17, rank 1). Fix in the ranking/prediction pipeline (`lucy lsd rank` + `lucy predict c13`): RDKit MolFromSmiles→sanitize/aromatize (or MolToSmiles canonical) before HOSE generation so Kekulé input is handled transparently; + regression test (Kekulé benzoate predicts identically to its canonical aromatic form); + solution-analyst skill note. NOTE: this is the ONLY confirmed defect from the successful Opus-4.8 run — the interpretive concern that motivated a candidate "FIX-11" earlier (not generating the benzylic alternative / rationalizing away evidence) did NOT recur on Opus 4.8 and is treated as a resolved model-capability gap, not a skill defect. (depends on Phase 83) (DONE + VERIFIED 2026-06-16: 5/5 SC. prepare_mol enforces Chem.SetAromaticity (no AddHs); genuine guard test — un-aromatized mol 0→6 aromatic restored; honest pin test; ranker.py:66/160 FIX-11 scope comments; compound-agnostic solution-analyst note (FIX-09-clean); suite 1081 passed. Library path was already correct → hardening+pin+skill-note, the real incident fix being the skill note. See 84-VERIFICATION.md.)

**Plans:** 1 plan (Wave 1)
Plans:
- [x] 84-01-PLAN.md — harden prepare_mol to enforce aromaticity perception (Kekulé→aromatic) + Kekulé==canonical regression test + compound-agnostic solution-analyst note (FIX-11)
- [x] **Phase 85: HMBC Peak-Picking Integrity (FIX-12)** - HMBC analog of FIX-08: `lucy pick hmbc` thresholds only on fraction-of-max (`-t`, default 0.05); no `--snr-floor`. Intense aliphatic/2J cross-peaks set the global max, so weak-but-real long-range correlations fall below 5%×max and are discarded. Proven on CASE1 (ibuprofen, HMBC expno 7): the ring-diagnostic 3J-meta cross-peaks H4→C2 (C≈141/H≈7.11) and H6→C3 (C≈137/H≈7.23) sit at ~0.6% of max vs ~22% for the 2J-ortho — ~35× weaker, ~8× below the default; only visible at `-t 0.005` amid 165 noise peaks (no clean gap). This is very likely the REAL emergent-ring blocker (D-04): recovering H4→C2 + H6→C3 would pin all 6 ring edges (2J-ortho + COSY CH–CH + 3J-meta + 2C-symmetry) so the benzene ring emerges WITHOUT forced ring-BONDs. Fix: add a noise-relative SNR floor to the HMBC picker (keep SNR≥floor cross-peaks even at <1% of max; reject ~0.5–2% noise), expose `--snr-floor` in `lucy pick hmbc`, consider per-row/region normalization; CASE1 regression (3J-meta retained above floor, noise out); nmr-chemist skill note. Directly targets emergent ring (D-04). See .planning/research/D04-emergent-ring/. (depends on Phase 81 peak-picker; relates to D-04) (DONE + VERIFIED 2026-06-16: 5/5 SC. SNR-floor 2D picker + `lucy pick hmbc --snr-floor` + guided routing + Peak2D.snr; CASE1 expno-7 — H4→C2 retained at SNR 22.4, H6→C3 at SNR 9.8, both DROPPED by legacy fraction-of-max; synthetic separation test; nmr-chemist Pitfall 12 (FIX-09-clean); suite 1079 passed. Downstream: live blind CASE run to confirm the benzene ring now emerges WITHOUT forced ring-BONDs. See 85-VERIFICATION.md.)

**Plans:** 2 plans (Wave 1, parallel — no file overlap)
Plans:
- [x] 85-01-PLAN.md — 2D HMBC SNR-floor picker (MAD noise) + Peak2D.snr + `lucy pick hmbc --snr-floor` + guided-HMBC default routing (FIX-12 a/b/c)
- [x] 85-02-PLAN.md — CASE1 expno-7 3J-meta retention regression + synthetic separation test + compound-agnostic nmr-chemist SNR note (FIX-12 d/e, FIX-09 guard)
...
- [x] 83-01-PLAN.md — Skill edits: nmr-chemist advisory rule + 145-160 ppm ambiguity, lsd-engineer FIX-10 prohibition, devils-advocate G-PROP-EVIDENCE gate
- [x] 83-02-PLAN.md — Tooling: NeighbourResult advisory field + CLI advisory note + regression tests (advisory label + CASE9-truth-not-excluded)
