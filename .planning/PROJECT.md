...
---
*Last updated: 2026-07-04 — v9.2 Phase 91 complete (webview API endpoints `/api/status|/api/log|/api/structures|/api/structure/{i}.svg` with graceful degradation + RDKit SVG depictions + single-file vanilla-JS auto-refresh dashboard served at `GET /`; static assets shipped in the wheel via hatch artifacts; WV-03/04/05/06 validated, WV-08 import-safety held); next up Phase 92 (orchestrator integration — auto-launch the dashboard from case.md, WV-07)*
